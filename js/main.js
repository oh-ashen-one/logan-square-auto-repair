...
function animateIfExists(selector, props, trigOpts={}) {
...
if (heroEyebrow) {
  ...
  tl.to('.hero-eyebrow', { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out' })
    ...
    .from('.hero-right img', { scale: 1.1, duration: 1.5, ease: 'power2.out' }, '-=1.2');
}

// Sticky shrink nav
const nav = document.querySelector('.site-nav');
if (nav) {
  ScrollTrigger.create({ start: 'top -60', onEnter: () => nav.classList.add('shrunk'), onLeaveBack: () => nav.classList.remove('shrunk') });
}
...
gsap.utils.toArray('section').forEach(s => {
  const t = s.querySelector('h2'); if (t && !s.classList.contains('hero')) gsap.from(t, { opacity: 0, y: 30, duration: 0.7, ease: 'power3.out', scrollTrigger: { trigger: s, start: 'top 82%', once: true } });
});
